...
from app.providers.eodhd import check_m5, EodhdError, rate_limit_status
from app.services.market_data import ensure_m5, ensure_daily, intraday_bars, daily_bars, metrics_for_bar, metrics_for_daily_bar
...
def settings_status():
    return {
        ...
        'm5_history_start': 'US-Aktien m5 laut EODHD ab Oktober 2020; im Tool: vor 2020-10-01 keine Intraday-Features.',
        'eodhd_rate_limit': rate_limit_status()
    }
...
async def chart(req: ChartRequest):
    try:
        ...
        if req.timeframe in ['daily','weekly']:
            bars = daily_bars(req.ticker, req.date, playback=True, timeframe='weekly' if req.timeframe == 'weekly' else 'daily')
            index = req.selected_index if req.selected_index is not None else len(bars)-1
            metrics = metrics_for_daily_bar(bars, index)
        else:
